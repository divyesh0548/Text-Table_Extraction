...
def add_proper_headers(df):
    """
    Add proper column headers to the DataFrame
    """
    headers = [
        "Pay Doc No",
        "Payment Type", 
    ...
    if len(df.columns) == len(headers):
        ...
        print(f"Warning: DataFrame has {len(df.columns)} columns but {len(headers)} headers expected")
        # Use headers for available columns, fill rest with generic names
        new_headers = headers[:len(df.columns)]
        if len(df.columns) > len(headers):
            new_headers.extend([f"Column_{i}" for i in range(len(headers), len(df.columns))])
...
# Load the merged data and add proper headers
merged_df = pd.read_excel("extracted_tables.xlsx", header=None)
print(f"\nMerged data shape: {merged_df.shape}")

# Add proper headers
final_df = add_proper_headers(merged_df)

# Save the final output with headers
final_df.to_excel("extracted_tables_with_headers.xlsx", index=False)
print(f"\nFinal output with headers saved to 'extracted_tables_with_headers.xlsx'")
print(f"Final data shape: {final_df.shape}")
print("\nFirst few rows with headers:")
print(final_df.head())
