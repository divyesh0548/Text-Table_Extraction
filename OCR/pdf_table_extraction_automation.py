...
def cleanup_temp_files():
    ...
    temp_patterns = ['temp_pdf_images', 'temp_page_*.jpg', 'temp_*.png', '*_temp.pdf']

    # Remove temp directories
    if os.path.exists('temp_pdf_images'):
        shutil.rmtree('temp_pdf_images', ignore_errors=True)
        print("[CLEANUP] Removed temp_pdf_images directory")

    # Remove temp files
    current_dir = os.getcwd()
    for filename in os.listdir(current_dir):
        if ('temp_page_' in filename and filename.endswith('.jpg')) or            ('temp_' in filename and filename.endswith('.png')) or            filename.endswith('_temp.pdf'):
            try:
                os.remove(filename)
...
def get_user_input():
    ...
    print("=== PDF Table Extraction Automation ===\n")
...
def extract_tables_with_easyocr(pdf_path, pages=None):
    """Extract tables using EasyOCR (img2table)"""
    print(f"\n[EXTRACT] Starting table extraction...")
    ...
    try:
        ...
        pdf_doc.to_xlsx(dest=excel_filename, ocr=ocr)
        print(f"[EXTRACT] Tables saved to: {excel_filename}")

        # Build list of dataframes for summary
        dataframes_list = []
        ...
        # Print summary
        print(f"[EXTRACT] Total tables extracted: {len(dataframes_list)}")
        for item in dataframes_list:
            df = item['dataframe']
            page = item['page']
            table_num = item['table_num']
            if not df.empty:
                print(f"  - Page {page}, Table {table_num}: {df.shape[0]} rows × {df.shape[1]} columns")

        return excel_filename, len(dataframes_list)
...
def main():
    ...
    try:
        ...
        excel_file, table_count = extract_tables_with_easyocr(current_pdf, pages_to_process)
